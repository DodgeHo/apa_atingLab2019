...
library(stringr)
library(ggplot2)
library(reshape)  # 注意：reshape 包的 melt 函数可能会被 data.table 覆盖
library(handy)
library(argparse)

# 确保使用 data.table 的 melt 函数
...
# Load histogram CSVs
a_hists <- dir(args$input_dir, pattern = "AsInWindow.csv", full.names = TRUE)
dat <- lapply(a_hists, fread)
names <- str_replace(str_replace(a_hists, ".*/", ""), "_AsInWindow.*", "")
names(dat) <- names
a_dat <- dat

t_hists <- dir(args$input_dir, pattern = "TagLenghAfterACut.csv", full.names = TRUE)
...
write.csv(summary, row.names = FALSE, file = sprintf("%s/ChopATailsResults.csv", args$output_dir))
...
ggplot(df, aes(x = file, y = value, fill = variable)) +
  geom_bar(stat = "identity") +
  handy::ggnice() +
  coord_flip() +
  labs(title = "Number of Reads with Each Cutting Decision", x = "FASTQ File", y = "Number of Reads") +
  scale_fill_manual(values = c("#4daf4a", "#377eb8", "#e41a1c"))

ggplot(df, aes(x = file, y = frac, fill = variable)) +
  geom_bar(stat = "identity") +
  handy::ggnice() +
  coord_flip() +
  labs(title = "Proportion of Reads with Each Cutting Decision", x = "FASTQ File", y = "Fraction of Total Reads") +
  scale_fill_manual(values = c("#4daf4a", "#377eb8", "#e41a1c")) +
...
ggplot(adt, aes(x = AsInWindow, y = File, fill = Frac)) +
  geom_tile() +
  handy::ggnice() +
  scale_x_continuous(breaks = seq(-1, 10, by = 1)) +
  scale_fill_gradient2(low = "#3288bd", mid = "#e6f598", high = "#d53e4f") +
  labs(title = "Distributions of Number of As in 10bp Window for all FASTQs")

for (i in 1:length(a_dat)) {
  print(ggplot(a_dat[[i]], aes(x = AsInWindow, y = Count)) +
          geom_bar(stat = "identity") +
          handy::ggnice() +
          scale_x_continuous(breaks = seq(-1, 10, by = 1)) +
          labs(title = paste0("As in 10bp Window: ", names(a_dat)[i])))
}
dev.off()
...
ggplot(tdt, aes(x = Length, y = File, fill = Frac)) +
  geom_tile() +
  handy::ggnice() +
  scale_x_continuous(breaks = seq(min(tdt$Length), max(tdt$Length), by = 5)) +
  scale_fill_gradient2(low = "#3288bd", mid = "#e6f598", high = "#d53e4f") +
  labs(title = "Distribution of Inferred Cleavage Site Location in Read for all FASTQs")

ggplot(tdt[!str_detect(tdt$File, "OTHER"), ], aes(x = Length, y = File, fill = Frac)) +
  geom_tile() +
  handy::ggnice() +
  scale_x_continuous(breaks = seq(min(tdt$Length), max(tdt$Length), by = 5)) +
  scale_fill_gradient2(low = "#3288bd", mid = "#e6f598", high = "#d53e4f") +
  labs(title = "Distribution of Inferred Cleavage Site Location in Read for non-other FASTQs")

for (i in 1:length(t_dat)) {
  print(ggplot(t_dat[[i]], aes(x = Length, y = Count)) +
          geom_point() +
          geom_line() +
          handy::ggnice() +
          geom_vline(xintercept = 18) +
          labs(title = paste0("Cleavage Site in Read Location Density: ", names(t_dat)[i])))
}
dev.off()
