...
def checkhex(seq,dorev):
	...
	for checkhex in hexes:
		#mpos = seq.find(checkhex)
		mpos = [m.start() for m in re.finditer(checkhex,seq)]
		if mpos:
			hex_loc[checkhex] = mpos
		#if mpos > -1:
		#	hex_loc[checkhex] = mpos
	#print "Found hex: " + out
	return hex_loc
				...
				for hex2,offsets in myhex.items():
					...
					if args.doprint and hex2!='NONE':
						for offset in offsets:
							hex_pos = f1_start + offset
							print_a.write("%s\t%s\t%d\t%d\t%s\n"%(hex2,my_chr,hex_pos,hex_pos+6,my_strand))
